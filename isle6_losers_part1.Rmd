...
```{r dp1_05, eval = F, message=F, warning=F}
...
## Remoive negated forms et al.{-}

```{r dp1_22, message=F, warning=F}
...
```{r dp1_24, eval = F, message=F, warning=F}
nrow(wscadjdf)
...
```{r dp1_26, eval = F, message=F, warning=F}
# inspect adjectives
names(table(wscadjdf$Adjective))
```

Remove words not in dictionary or consist only of 1 or 2 characters

```{r dp1_27, message=F, warning=F}
# correct spelling
library(hunspell)
wscadjdf <- wscadjdf %>%
...
```{r dp1_29, eval = F, message=F, warning=F}
# inspect adjectives
names(table(wscadjdf$Adjective))
```
...
```{r isle6_1_59, message=F, warning=F}
wscadjdf %>%
...
```{r intro41, echo = F, message=FALSE, warning=FALSE}
# inspect data
wscadjdf %>%
  head(20) %>%
  kable(caption = "First 20 rows of wscadjdf.") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                            full_width = F)
```


```{r dp1_44, message=F, warning=F}
# save raw data to disc
