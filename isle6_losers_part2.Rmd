...
```{r isle6_2_01, message=F, warning=F}
# load packages
library(tidyverse)
library(tidyr)
# set options
...
```{r isle6_2_02, message=F, warning=F}
exdata <- ampwsc %>%
...
```{r isle6_2_03, message=F, warning=F}
#                  CLEAN DATA
ampwsc <- ampwsc %>%
#  dplyr::mutate(Type = dplyr::case_when(Type == "BroadcastMonologue" ~ "formal",
#                                        Type == "BroadcastNews" ~ "formal",
#                                        Type == "BroadcastWeather" ~ "formal",
#                                        Type == "JudgesSummation" ~ "formal",
#                                        Type == "Lecture" ~ "formal",
#                                        Type == "ParliamentaryDebate" ~ "formal",
#                                        Type == "TeacherMonologue" ~ "formal",
#                                        Type == "TransactionsAndMeetings" ~ "formal",
#                                        TRUE ~ "informal")) %>%
  dplyr::select(-Text, -CleanText, -TextPOS, -OriginalString, 
                -ExtractMinutes, -WordsFile, -PreContext, -PostContext,
                -Minutes, -PreContextLong, -Id, - Topic, -Words) %>%
  na.omit() %>%
  # recode age
...
```{r isle6_2_04, message=F, warning=F}
# recode education
...
```{r isle6_2_05, message=F, warning=F}
# inspect data
ldtb <- ampwsc %>%
  dplyr::select(Age, Adjective, Variant, Function) %>%
  dplyr::group_by(Age, Variant, Function) %>%
  dplyr::summarize(AdjectiveTypes = length(table(Adjective)), 
                   VariantN = n(),
                   AllTypes = sum(table(Adjective))) %>%
  dplyr::filter(Variant == "very"|Variant == "really"|Variant == "so"|Variant == "pretty") %>%
  dplyr::mutate(LD = AdjectiveTypes/VariantN)
# inspect
ldtb
```
...
```{r}
ldtb %>%
  ggplot(aes(x = Age, y = LD, group = Variant, color = Variant)) +
  geom_line() + 
  facet_grid(~Function) +
  theme_bw() +
...
```{r isle6_2_06, message=F, warning=F}
ampwsc %>%
...
```{r isle6_2_07, message=F, warning=F}
ampwsc %>%
  dplyr::select(Age, Amplified, Variant) %>%
  ...
  theme(legend.position = "top",
        ...
        panel.grid.minor = element_blank(), 
        axis.line = element_line(colour = "black")) +
  ggsave(file =  here::here("images", "Percent_Variant_Age.png"), 
         width = 15, height = 10, units = c("cm"),  dpi = 600)
```


```{r isle6_2_08, message=F, warning=F}
ampwsc %>%
...
```{r isle6_2_09, message=F, warning=F}
ampwsc %>%
  dplyr::select(Age, Function, Amplified, Variant) %>%
  dplyr::filter(Amplified != "0") %>%
  dplyr::mutate(Variant = dplyr::case_when(Variant == "very" ~ "very",
...
```{r isle6_2_10, message=F, warning=F}
...
```{r isle6_2_11, message=F, warning=F}
p2d %>%
...
```{r isle6_2_12, message=F, warning=F}
wordcounts <- read.delim(here::here("data", "wordcount.txt"), sep = "\t", header = T, skipNul = T) %>%
  dplyr::filter(grepl("DPC", File) == T)
...
```{r isle6_2_13, message=F, warning=F}
p3d %>%
  ggplot(aes(x = reorder(Age, desc(Age)), y = Frequency, group = Variant, 
             shape = Variant, color = Variant, linetype = Variant)) +
  facet_grid(~Function) +
...
```{r isle6_2_14, message=F, warning=F}
# save data to disc
...
```{r isle6_2_15, message=F, warning=F}
# clean data
ampwsc <- ampwsc %>%
  dplyr::select(-Token)  %>%
...
```{r isle6_2_16, message=F, warning=F}
tb1dat <- ampwsc
hfvars <- names(table(tb1dat$Variant))[which(table(tb1dat$Variant) > 10)]
tb1dat <- tb1dat %>%
  dplyr::mutate(Variant = ifelse(Variant %in% hfvars, Variant, "other"))

# tb 1
Varianttbwsc <- table(tb1dat$Variant)
Varianttbwsc <- Varianttbwsc[order(table(tb1dat$Variant), decreasing = T)]
...
```{r isle6_2_17, message=F, warning=F}
# save data to disc
write.table(tb1, here::here("data", "Table1.txt"), sep = "\t", row.names = F)
...
```{r isle6_2_18, message=F, warning=F}
# create table
Table2 <- tb2dat %>%
...
```{r isle6_2_19, message=F, warning=F}
# save data to disc
...
```{r isle6_2_20, message=F, warning=F}
# save data to disc
...
```{r isle6_2_21, message=F, warning=F}
sessionInfo()
```
