---
title: "On the waning of forms - a corpus-based analysis of losers in language change (Part 4)"
...
This document documents the analysis of the adjective amplifier *really* in New Zealand English. The analysis was performed with the aim of fitting a generalized-linear binomial logistic mixed-effects regression to explain which factors correlate with the use of *really* and to ascertain if 

1. *really* is increasing across apparent time 

2. if the increase of *really* is systematic or layered (which would imply that the use of *really* is stratified along linguistic and social lines) or unsystematic (which would imply that it correlates with few if any linguistic or social variables). 

# Introduction

We will now begin with the analysis. In a first step, the session is prepared by setting options and activating packages.

```{r isle6_4_01, message=F, warning=F}
# load packages
library(tidyverse)
library(tidyr)
# set options
...
```{r isle6_4_02, message=F, warning=F}
# load data
reallywsc <- read.delim(here::here("data", "ampwsc_regdat.txt"), sep = "\t", header = T, skipNul = T)
# remove superfluous variables
reallywsc <- reallywsc %>%
    dplyr::filter(Variant != "0") %>%
  dplyr::select(-ID, -FileSpeaker, -Speaker, -very, -File, -Age_original) %>%
...
# inspect data
nrow(reallywsc); str(reallywsc); head(reallywsc); summary (reallywsc)
```
...
```{r isle6_4_03, message=F, warning=F}
# frequency
...
```{r isle6_4_05, message=F, warning=F}
plot(reallywsc$Frequency)
```

We also scale gradability and plot the resulting scaled gradability.

```{r isle6_4_07, message=F, warning=F}
# gradability
reallywsc <- reallywsc %>%
...
plot(reallywsc$Gradability)
```

# CIT

```{r isle6_4_08, message=F, warning=F}
# load packages
library(partykit)
library(tidyverse)
# create data
...
citd.ctree <- partykit::ctree(really ~ Age +  SemanticCategory + Frequency +  Priming + 
                                Type + Emotionality  + Gender + L1 + Function, data = citdata)
plot(citd.ctree, gp = gpar(fontsize = 8)) # plot final ctree
```

```{r isle6_4_09, message=F, warning=F}
citd.ctree
```
...
```{r isle6_4_10, message=F, warning=F}
reallywsc <- reallywsc %>%
  # simplify age
...
# inspect
table(reallywsc$SemanticCategory)
table(reallywsc$Age)
table(reallywsc$Type)
table(reallywsc$Ethnicity)
```

```{r isle6_4_11, message=F, warning=F}
# create data
citdata <- reallywsc
# set seed
set.seed(111) 
...
```{r isle6_4_12, message=F, warning=F}
# activate package
library(ggparty)
# extract p-values
...
ggparty(citd.ctree) +
  ...
  geom_node_plot(gglist = list(
    ...
    xlab(""), 
    ylab("Probability"),
    geom_text(aes(x = "", group = really,
                  label = stat(count)),
              stat = "count", position = position_fill(), vjust = 1.1)),
    shared_axis_labels = TRUE) +
  ggsave(file =  here::here("images", "Cit_really.png"), 
...
```{r isle6_4_13, message=F, warning=F}
# load library
library(Boruta)
...
```{r isle6_4_14, message=F, warning=F}
...
```{r isle6_4_15, message=F, warning=F}
...
ggplot(borutadf, aes(x = reorder(Variable, Importance, mean), y = Importance, fill = Type)) + 
  ...
  labs(x = "Variable") +
  ggsave(file =  here::here("images", "Boruta_really.png"), 
...
```{r isle6_4_19, message=F, warning=F}
# load library
library(rms)
# set options
...
m0.glm = glm(really ~ 1, family = binomial, data = reallywsc) 
# inspect results
summary(m0.glm)
```
...
```{r isle6_4_20, message=F, warning=F}
# load packages
library(lme4)
library(car)
...
```{r isle6_4_21, message=F, warning=F}
...
Probability of really among speakers aged 40+
...
```{r isle6_4_24, message=F, warning=F}
pm1 <- m1.glmer
...
```{r isle6_4_25, message=F, warning=F, eval = F}
...
```{r isle6_4_32}
ifelse(min(ftable(reallywsc$Type, reallywsc$really)) == 0, "not possible", "possible")
m5.glmer <- update(m3.glmer, .~.+Type)
car::vif(m5.glmer)
...
m8.glmer <- update(m7.glmer, .~.+Ethnicity)
car::vif(m8.glmer)
...
```{r isle6_4_39}
ifelse(min(ftable(reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible")
m9.glmer <- update(m7.glmer, .~.+Education)
car::vif(m9.glmer)
...
```{r isle6_4_41}
m10.glmer <- update(m7.glmer, .~.+Gradability)
car::vif(m10.glmer)
...
```{r isle6_4_43, message=F, warning=F}
# find all 2-way interactions
library(utils)
#colnames(reallywsc) # remove # to activate
...
m12.glmer <-  update(m7.glmer, .~.+ Age * SemanticCategory)
max(car::vif(m12.glmer)[,1])
# excessive vifs
```

```{r isle6_4_m13}
m13.glmer <-  update(m7.glmer, .~.+ Age * Frequency)
max(car::vif(m13.glmer)[,1])
anova(m13.glmer, m7.glmer, test = "Chi") 
```

```{r isle6_4_m13p, eval = F}
pm13 <- m13.glmer
...
```{r isle6_4_m16}
ifelse(min(ftable(reallywsc$Age, reallywsc$Type, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m19}
ifelse(min(ftable(reallywsc$Age, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible")
...
```{r isle6_4_m23}
ifelse(min(ftable(reallywsc$SemanticCategory, reallywsc$Priming, reallywsc$really)) == 0, "not possible", "possible")
...
```{r isle6_4_m25}
ifelse(min(ftable(reallywsc$SemanticCategory, reallywsc$Type, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m26}
ifelse(min(ftable(reallywsc$SemanticCategory, reallywsc$Gender, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m28}
ifelse(min(ftable(reallywsc$SemanticCategory, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m34}
m34.glmer <-  update(m14.glmer, .~.+ Frequency * Gender)
max(car::vif(m34.glmer)[,1])
anova(m34.glmer, m14.glmer, test = "Chi")
```

```{r isle6_4_m34p, eval = F}
pm34 <- m34.glmer
...
```{r isle6_4_m35}
m35.glmer <-  update(m14.glmer, .~.+ Frequency * Ethnicity)
max(car::vif(m35.glmer)[,1])
anova(m35.glmer, m14.glmer, test = "Chi")
```

```{r isle6_4_m35p, eval = F}
pm35 <- m35.glmer
...
```{r isle6_4_m36}
m36.glmer <-  update(m14.glmer, .~.+ Frequency * Education)
max(car::vif(m36.glmer)[,1])
anova(m36.glmer, m14.glmer, test = "Chi")
```

```{r isle6_4_m36p, eval = F}
pm36 <- m36.glmer
...
```{r isle6_4_m37}
m37.glmer <-  update(m14.glmer, .~.+ Frequency * Gradability)
max(car::vif(m37.glmer)[,1])
anova(m37.glmer, m14.glmer, test = "Chi") 
```

```{r isle6_4_m37p, eval = F}
pm37 <- m37.glmer
...
m39.glmer <-  update(m14.glmer, .~.+ Priming * Emotionality)
max(car::vif(m39.glmer)[,1])
# excessive vifs
```

```{r isle6_4_m40}
ifelse(min(ftable(reallywsc$Priming, reallywsc$Type, reallywsc$really)) == 0, "not possible", "possible") 
m40.glmer <-  update(m14.glmer, .~.+ Priming * Type)
...
m41.glmer <-  update(m14.glmer, .~.+ Priming * Gender)
max(car::vif(m41.glmer)[,1])
anova(m41.glmer, m14.glmer, test = "Chi")
Anova(m41.glmer, type = "III", test = "Chi") 
```

```{r isle6_4_m41p, eval = F}
pm41 <- m41.glmer
...
```{r isle6_4_m43}
ifelse(min(ftable(reallywsc$Priming, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
Anova(m44.glmer, type = "III", test = "Chi") 
```

```{r isle6_4_m44p, eval = F}
pm44 <- m44.glmer
...
```{r isle6_4_m46}
ifelse(min(ftable(reallywsc$Emotionality, reallywsc$Type, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m47}
ifelse(min(ftable(reallywsc$Emotionality, reallywsc$Gender, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m48}
ifelse(min(ftable(reallywsc$Emotionality, reallywsc$Ethnicity, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m49}
ifelse(min(ftable(reallywsc$Emotionality, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
anova(m50.glmer, m41.glmer, test = "Chi")
```

```{r isle6_4_m52}
ifelse(min(ftable(reallywsc$Type, reallywsc$Gender, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m53}
ifelse(min(ftable(reallywsc$Type, reallywsc$Ethnicity, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m54}
ifelse(min(ftable(reallywsc$Type, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m58}
ifelse(min(ftable(reallywsc$Gender, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_m62}
ifelse(min(ftable(reallywsc$Ethnicity, reallywsc$Education, reallywsc$really)) == 0, "not possible", "possible") 
...
```{r isle6_4_44, message=F, warning=F}
...
```{r isle6_4_46,  message=F, warning=F}
...
m8m7 <- anova(m8.glmer, m7.glmer, test = "Chi") 
m9m7 <- anova(m9.glmer, m7.glmer, test = "Chi") 
m10m7 <- anova(m10.glmer, m7.glmer, test = "Chi")   
m13m7 <- anova(m13.glmer, m7.glmer, test = "Chi") 
m14m7 <- anova(m14.glmer, m7.glmer, test = "Chi") 
...
m34m14 <- anova(m34.glmer, m14.glmer, test = "Chi")
m35m14 <- anova(m35.glmer, m14.glmer, test = "Chi")
m36m14 <- anova(m36.glmer, m14.glmer, test = "Chi")
m37m14 <- anova(m37.glmer, m14.glmer, test = "Chi") 
m41m14 <- anova(m41.glmer, m14.glmer, test = "Chi")
m44m41 <- anova(m44.glmer, m41.glmer, test = "Chi")
m50m41 <- anova(m50.glmer, m41.glmer, test = "Chi")
...
mdlcmp <- list(m1m0, m2m1, m3m1, m4m3, m5m3, m6m5, m7m6, m8m7, m9m7, m10m7, m13m7, m14m7, 
               m20m14, m31m14, m33m14, m34m14, m35m14, m36m14, m37m14, m41m14, m44m41, 
               m50m41, m58m41, m59m58, m64m58)
# load function
...
write.table(mdl.cmp.glmersc.swsu.dm, here::here("data", "mdl_cmp_glmersc_swsu_reallywscnew.txt"), sep="\t")
# inspect output
...
```{r isle6_47_glm}
m58.glm <-  glm(really ~ Age + Priming + Type + Gender + SemanticCategory +  
    Education + Age:Priming + Priming:Gender + Gender:Education, 
    family = "binomial", data = reallywsc)
```

```{r isle6_4_48, message=F, warning=F}
# load function for regression table summary
source("D:\\R/meblr.summary.R")
# set up summary table
meblrm_ampwsc <- meblrm.summary(m0.glm, m58.glm, m0.glmer, m58.glmer, reallywsc$really) 
# save results to disc
write.table(meblrm_ampwsc, here::here("data", "meblrm_ampwsc_really.txt"), sep="\t")
...
```{r isle6_4_49, message=F, warning=F}
# load function
library(car)
meblrm_ampwsc_Anova <- Anova(m58.glmer, type = "III", test = "Chi")
# save results to disc
write.table(meblrm_ampwsc_Anova, here::here("data", "meblrm_ampwsc_Anova_really.txt") , sep="\t")
...
```{r isle6_4_50, message=F, warning=F}
sjPlot::plot_model(m58.glmer) +
  ...
  labs(title = "Odds Ratio of really")
  ggsave(file =  here::here("images", "Effectsreally.png"), 
         width = 20, height = 10, units = c("cm"),  dpi = 600)
```

```{r isle6_4_51, message=F, warning=F}
sjPlot::tab_model(m0.glmer, m58.glmer,
                  dv.labels = c("Intercept-only Model", "Final Minimal Model"))
```

Inspect probabilities

```{r isle6_4_52, message=F, warning=F}
...
```{r isle6_4_53, message=F, warning=F}
library (multcomp)
...
```{r isle6_4_54, message=F, warning=F}
summary(glht(m58.glmer, mcp(SemanticCategory="Tukey")))
```
...
```{r isle6_4_55, message=F, warning=F}
# predict probs of nativelike for effects
reallywsc$PredictedFrequency <- predict(m58.glmer, reallywsc, type="response")
...
```{r isle6_4_56, message=F, warning=F}
# predict probs of nativelike for effects
reallywsc$PredictedFrequency <- predict(m1.glmer, reallywsc, type="response")
# create response variable
reallywsc$PredictedResponse <- ifelse(reallywsc$PredictedFrequency > .5, "really", "other")
#summary(reallywscnew$PredictedResponse)
# load library
...
```{r isle6_4_58, message=F, warning=F}
...
```{r isle6_4_59, message=F, warning=F}
sjPlot::plot_model(m58.glmer, type = "pred", terms = c("Age", "Priming")) +
  ...
  scale_y_continuous(name = "Predicted Probabilty of really", 
                     limits = c(0, 1),
                     breaks = seq(0, 1, .20), 
                     labels = seq(0, 1, .20)) +
  labs(title = "") +
  ggsave(file =  here::here("images", "Effect_Age_Priming_really.png"), 
         width = 7.5, height = 15, units = c("cm"),  dpi = 600)
```

```{r isle6_4_60, message=F, warning=F}
sjPlot::plot_model(m58.glmer, type = "pred", terms = c("Gender", "Priming")) +
  theme_bw() +
  theme(legend.position = "top",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
  ...
  scale_y_continuous(name = "Predicted Probabilty of really", 
                     ...
                     labels = seq(0, 1, .20)) +
  labs(title = "") +
  ggsave(file =  here::here("images", "Effect_Gender_Priming_really.png"), 
         width = 7.5, height = 15, units = c("cm"),  dpi = 600)
```

```{r isle6_4_61, message=F, warning=F}
sjPlot::plot_model(m58.glmer, type = "pred", 
  ...
  scale_y_continuous(name = "Predicted Probabilty of really", 
                     ...
                     labels = seq(0, 1, .20)) +
  labs(title = "") +
  ggsave(file =  here::here("images", "Effect_Gender_Education_really.png"), 
         width = 7.5, height = 15, units = c("cm"),  dpi = 600)
```

```{r isle6_4_62, message=F, warning=F}
sjPlot::plot_model(m58.glmer, type = "pred", terms = c("SemanticCategory")) +
  theme_bw() +
  theme(legend.position = "top",
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), 
  ...
  scale_y_continuous(name = "Predicted Probabilty of really", 
                     ...
                     labels = seq(0, 1, .20)) +
  labs(title = "") +
  ggsave(file =  here::here("images", "Effect_SemanticCategory_really.png"), 
         width = 10, height = 10, units = c("cm"),  dpi = 600)
```

```{r isle6_4_63, message=F, warning=F}
sjPlot::plot_model(m58.glmer, type = "pred", terms = c("Type")) +
  ...
  scale_y_continuous(name = "Predicted Probabilty of really", 
                     ...
                     labels = seq(0, 1, .20)) +
  labs(title = "") +
  ggsave(file =  here::here("images", "Effect_Type_really.png"), 
         width = 10, height = 10, units = c("cm"),  dpi = 600)
```

```{r isle6_4_64, message=F, warning=F}
...
```{r isle6_4_65, message=F, warning=F}
ggplot(rndmadjtb, aes(Adjective, Intercept)) +
  ...
  ggsave(file = here::here("images", "RandomEffects_Adjective_really.png"), 
       height = 5,  width = 7.5,  dpi = 320)
```

```{r isle6_4_66, message=F, warning=F}
rndmadjtb %>%
  ...
  theme(legend.position="none", 
        ...
        panel.grid.minor = element_blank()) +
  labs(x = "Adjective type \n(only selected labels displayed)", y = "Adjustment to Intercept") +
  ggsave(file = here::here("images", "RandomEffects_Adjective_redux_really.png"), 
       height = 5,  width = 7.5,  dpi = 320)
```

# Citation & Session Info {-}

Schweinberger, Martin. `r format(Sys.time(), '%Y')`. *On the waning of forms - a corpus-based analysis of losers in language change (Part 4)*. Brisbane: The University of Queensland. url: https://slcladal.github.io/isle6verynze.html (Version `r format(Sys.time(), '%Y.%m.%d')`).
